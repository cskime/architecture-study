...
- UI, DB 외에도 다양한 요소가 변경의 축이 될 수 있다.
- 변경될 수 있는 요소들을 **추상 컴포넌트**를 사용하여 업무 규칙으로부터 분리해 낼 수 있다.
- 아키텍처 경계는 어디에서도 존재할 수 있고, 언제 필요한지 신중하게 판단해야 한다.
- **추상화가 필요하리라고 미리 예측해서는 안된다. => YAGNI(You Aren't Goint to Need It), Over Engineering**
- **'경계를 구현하는 비용 < 경계를 무시해서 발생하는 비용'** 이 되는 시점에 경계를 구현할 수 있도록 판단해야 한다.

## 26장. 메인 컴포넌트

- Main component
  - 모든 시스템에 최소 한 개는 존재하는 컴포넌트
  - 메인 컴포넌트가 다른 컴포넌트를 생성/조정/관리한다.
  - 가장 저수준의 컴포넌트, Entry point => `AppDelegate`?
- 의존성 주입은 main component에서 이루어 져야 하고, 이후에는 일반적인 방법으로 의존성을 분배할 수 있어야 한다.
- 이 모든 작업을 담당해야 하므로, 메인 컴포넌트는 가장 지저분한 컴포넌트이다.
- 메인 컴포넌트는 고수준의 시스템을 위한 모든 것을 로드한 뒤 제어권을 고수준 시스템에게 넘긴다.
- 메인 컴포넌트를 application의 '플러그인'으로 생각할 수 있다.
- 개발용, 테스트용, 상용 등 설정 별로 메인 컴포넌트 플러그인을 만들 수 있다.
- 메인 컴포넌트는 아키텍처 경계의 가장 바깥에 위치하므로, 아키텍처 내에서 설정 문제를 쉽게 해결할 수 있다.
  - _App에서 화면(모듈)이 전환되는 경우, 각 화면의 의존성 관계를 각 모듈 별 메인 컴포넌트에서 구성할 수 있겠다._
  - _AppDelegate에서 모든 화면에 대한 의존성 관계를 미리 설정해 두고, 각 모듈 별로 단순히 가져다 사용할 수 있겠다._
  - _Swinject같은 의존성 관리 라이브러리도 비슷한 개념으로 사용할 수 있겠다._

## 29장. 클린 임베디드 아키텍처

- 펌웨어와 하드웨어에 대한 의존성을 관리하지 않으면 소프트웨어도 내부에서부터 파괴될 수 있다.
- 펌웨어는 하드웨어에 의존하거나 하드웨어 발전에 맞춰 수정하기 어려운 소프트웨어로 정의할 수 있다.
  - SQL을 코드에 심어 놓거나 플랫폼 의존성을 가지는 등
- 소프트웨어를 개발하는 세 가지 활동
  - 동작하게 만든다
  - 올바르게 만든다 (리팩터링을 통해 사람들이 이해할 수 있고 쉽게 변경할 수 있게 만든다)
  - 빠르게 만든다 (리팩터링을 통해 요구되는 성능을 만족시킨다)
- 앱이 동작하게 만드는 것을 넘어서, 짧고 빠른 주기로 코드를 끊임없이 개선하고 최적화해야 한다.
- 임베디드 아키텍처의 계층
  - 소프트웨어
  ...
  - 펌웨어
  - 하드웨어
- 하드웨어와 펌웨어로부터 소프트웨어가 오염되는 것을 막기 위해 HAL 계층을 둔다.
  - 소프트웨어에서는 데이터가 디스크에 저장되는지 클라우드에 저장되는지 알지 못해야 한다.
- 소프트웨어가 운영체제에 의존하는 것을 막기 위해 OSAL 계층을 둔다.
  - 소프트웨어는 어떤 OS에서 실행될지 알지 못해야 한다.
  - OS에서 변경사항이 소프트웨어가 변경되는 것을 막는다.
- HAL과 OSAL은 소프트웨어 관점에서 필요한 기능으로 추상화한다.
- HAL과 OSAL은 임베디드 소프트웨어의 재사용성과 대체 가능성을 높이는 방법이다.

## 30장. 데이터베이스는 세부사항이다

- 데이터베이스는 데이터에 접근하는 방법을 제공하는 소프트웨어일 뿐, 아키텍처에서 데이터 구조와 데이터베이스는 다르다.
- 아키텍처에서는 데이터가 데이터베이스에서 행 단위로 저장되며 테이블 구조를 가진다는 것을 알아서는 안된다.
  - 행, record 등의 형태를 아키텍처 안에서 알게 되면 관계형 데이터베이스 구조에 강하게 결합되는 것
  - '어떤 데이터인지'를 제외한 데이터베이스의 다른 정보들에 대해 알면 안된다.
- 데이터베이스가 세부사항인 이유
  - 데이터베이스에서 데이터를 읽어들인 후, 대부분의 경우 데이터를 사용하기 편리한 형태의 구조로 변형하게 된다.
  - 즉, 데이터베이스는 데이터를 저장하고 그것에 쉽고 빠르게 접근하는 방법을 제공할 뿐이다.
  - 아키텍처 관점에서는 데이터가 데이터베이스에 있는지 디스크에 있는지 전혀 신경쓰지 않아야 한다.
  - 성능도 중요하지만 이것은 저수준에서 데이터에 접근할 때 처리할 일이고, 아키텍처와는 관련이 없다.
- 아키텍처 관점에서, **체계화된 데이터 구조와 데이터 모델은 중요하지만, 데이터를 옮길 뿐인 기술과 시스템은 중요하지 않다.**

## 31장. 웹은 세부사항이다

- Web을 핵심 업무 규칙의 중심에서 밀어내기 위한 시도가 계속되고 있다.
  - 브라우저가 연산을 담당하여 직접 렌더링하는 방식
  - 서버에서 모든 연산을 담당하고 클라이언트(브라우저)에서는 단순히 렌더링만 하는 방식
- 마케팅 천재는 어디에나 있으므로 UI와 업무규칙을 항상 서로 격리시키고 있어야 한다.
- Web은 UI이고, 이것은 핵심 업무 로직에서 분리되어 경계 바깥에 격리되어 있어야 한다.
- 업무 로직은 다수의 유스케이스로 구성되어 있고, 유스케이스는 입출력 데이터와 수행할 처리 과정으로 구성된다.
- 이 때, 유스케이스의 입출력 데이터를 독립적으로 완전한 구조로 만들어서 사용한다면 UI를 독립적으로 동작시킬 수 있다.

## 32장. 프레임워크는 세부사항이다

- 프레임워크는 내 문제를 해결하기 위해 만들어진 것이 아니고 그것에 관심도 없으므로 언제든 바뀔 수 있다.
  - 사용하던 기능이 변경되거나 삭제될 수 있다.
  - 더 이상 프레임워크를 사용하고 싶지 않아질 수 있다.
  - 더 나은 프레임워크가 등장해서 교체하고 싶을 수 있다.
- 프레임워크는 한 번 의존성을 갖고 사용하고 나면 다시 없애기 힘들기 때문에 직접적으로 강한 의존성을 가지면 안된다.
- 즉, 프레임워크를 아키텍처 가장 바깥의 세부사항으로 취급하라.
- 이 프레임워크는 proxy 컴포넌트를 통해서 핵심 코드에 플러그인 되어야 한다.
- 의존성 주입 등이 필요하다면 메인 컴포넌트에서 이루어져야 한다.
- 가능한 프레임워크를 아키텍처 경계 바깥에 두려고 시도해야 한다.

## 33장. 비디오 판매 애플리케이션 사례 연구

- 아키텍트의 아키텍처 설계 과정
  1. 액터와 유스케이스 식별
  2. 유스케이스 분석 -> 추상화가 필요한 컴포넌트 찾기
  3. 컴포넌트 아키텍처 설계
  4. 의존성 관리
- 핵심은 '액터 분리'와 '의존성 규칙'
- 서로 다른 이유(액터)와 서로 다른 속도(정책 수준)로 변경되는 컴포넌트를 분리하는 것이 핵심이다.

## 34장. 설계 및 코드 조직화와 관련된 접근법

1. 계층 기반 패키지 (수평적 계층화)
  - 해당 코드가 하는 일에 기반하여 코드 분리
  - 분리 예시
    - 'Orders' 계층에서 하는 일을 기반으로 분리
    - 하는 일 : 
      - 'web' module : `OrdersController`,
      - 'service' module : `OrdersService`(업무 규칙)
      - 'data' module : `OrdersRepository`(영속성)
  - 장점
    - 쉽고 빠르게 기능이 동작하도록 만들 수 있다.
  - 단점
    - 소프트웨어가 커지고 복잡해지면 더 작은 모듈로 분리하는 것을 고민하게 된다.
    - 업무 도메인에 대해 말해주지 않는다.
2. 기능 기반 패키지 (수직적 계층화)
  - 서로 연관된 기능, 도메인에 기반하여 수직의 얇은 조각으로 코드 분리
  - 계층 기반 패키지의 인터페이스 및 구현체를 모두 같은 module로 묶는다. (to 'Orders')
  - 분리 예시
    - 'orders' module로 controller, service, repository 등 주문 관련 코드를 모두 분리
  - 장점
    - 특정 도메인과 관련된 코드임을 알 수 있다. (e.g. '주문(Orders)'에 관련된 기능임을 알 수 있다.)
    - 특정 도메인과 관련된 기능(e.g. 주문 조회하기 usecase)이 변경될 경우, 어떤 코드를 변경해야 하는지 쉽게 찾을 수 있다.
3. 포트와 어댑터 (or Hexagonal Architecture)
  - 업무/도메인에 초점을 둔 코드가 프레임워크나 데이터베이스 같은 기술적 세부사항과 독립적이며 분리됨
  - 내부 모듈(도메인), 외부 모듈(인프라)로 구분하며 외부에서 내부로만 의존한다.
  - 분리 예시
    - 'web' module : `OrdersController`
    - 'domain' module : `OrdersService`, `Orders`
    - 'database' module : `JbdcOrdersRepository`
4. 컴포넌트 기반 패키지
  - 계층형 아키텍처의 문제
    - 엄격한 계층형 아키텍처에서는 의존성 화살표가 각 계층의 바로 아래 계층에만 의존해야 한다.
    - 각 module의 인터페이스를 'public'으로 선언해야 하므로, 의도하지 않는 의존 관계가 형성될 가능성이 존재한다.
    - 가령, `OrdersController`가 `OrdersService` 대신 `OrdersRepository`에 직접 의존할 수도 있다.
    - 개발자들끼리 규칙을 강제하거나, 정적 분석 도구를 사용하는 것에는 한계가 있다.
    - 따라서, **컴파일러를 사용해서 아키텍처를 강제하는 방식**이 필요한데, 그것이 컴포넌트 기반 패키지이다.
  - 컴포넌트 기반 패키지
    - 업무 로직과 영속성 관련 코드를 하나의 package로 묶는다. -> "컴포넌트"
    - 분리 예시
      - 'web' module : `OrdersController`
      - 'orders' module : `OrdersComponent`, `OrdersRepository`
    - 이 때, 각 component에서 입구 역할을 하는 인터페이스만 'public'으로 선언되므로 컴파일러의 도움을 받아서 컴포넌트 기반 아키텍처를 강제할 수 있다.
      - `OrdersController`가 `OrdersComponent`외에 다른 인터페이스 또는 클래스에 접근할 수 있는 방법이 컴파일 타임에 차단된다.
    - 장점
      - 특정 기능과 관련된 코드를 수정할 때, 오직 하나의 component만 둘러보면 된다.
5. 결론
  - 원하는 아키텍처를 설계할 때, 코드를 어떻게 조직화할지, 런타임/컴파일타임에 어떤 결합 분리 모드를 적용할지 고민해야 한다.
  - 선택된 아키텍처 스타일을 강제하는 데 컴파일러의 도움을 받을 수 있을지 고민한다.